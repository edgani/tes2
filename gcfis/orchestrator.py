...
def run_gcfis(prices: dict, bench: pd.Series, regime_posterior: dict,
              ...
              volumes=None, cot_by_ticker=None, leadlag_pairs=None, min_adv=0.0):
    ...
    # --- ENTRY (L13) attached per directional signal ---
    longs, shorts, spots = [], [], []
    for sig in ranking["signals"]:
        if sig.direction in ("long", "short"):
            ...
            if e.get("ok"):
                ...
                if e.get("warning"): sig.reason += f" | ENTRY: {e['warning']}"
        row = sig.as_dict()
        if sig.action == "BUILD_LONG" or (sig.action == "START_SCALING" and sig.direction == "long"):
            longs.append(row)
        elif sig.action == "BUILD_SHORT" or (sig.action == "START_SCALING" and sig.direction == "short"):
            shorts.append(row)
        if per_ticker[sig.ticker].get("sweet_spot") and sig.scores["meta_long"] >= 50:
            spots.append(row)
    ...
    return {"ok": True,
            "systemic": {"fragility": frag, "shock": shock, "forward_macro": fwd, "liquidity": liq,
                         "flow": flow, "theme": theme, "bottleneck": bott, "crypto": crypto},
