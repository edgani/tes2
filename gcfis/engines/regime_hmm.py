...
def _fallback(index_returns) -> dict:
    r = pd.to_numeric(pd.Series(index_returns), errors="coerce").dropna()
    if len(r) < 25:
    ...
    return {"ok": True, "method": "heuristic_fallback", "posterior": post, "state": max(post, key=post.get)}

def run_regime_hmm(index_returns, n_states: int = 5, breadth=None, xasset_corr=None, vix=None, seed: int = 42) -> dict:
    f = _features(index_returns, breadth, xasset_corr, vix)
    if not _HAVE_HMM or len(f) < 150:
        return _fallback(index_returns)
